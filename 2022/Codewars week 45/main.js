...
function rgb(r, g, b){
    let rgb = [r, g, b]
    let sanitizedRgb = rgb.map(color => { //sanitizing
        if(color<0){
            return 0
        }else if(color>255){
            return 255
    ...
    let hexa = sanitizedRgb.map(color => color.toString(16).toUpperCase()) //converting to hexa
    hexa = hexa.map(c => c.length===1 ? '0'+c : c) //adding leading zeroes

    return hexa.join('')
}


function rgbBis(r, g, b){
