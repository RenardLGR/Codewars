...
function listPosition(word) {
    ...
    function numberUniquePermutations(word){
        ...
        let res = factorial(word.length)
        for(let f in freq){
            res /= factorial(freq[f])
        }
...
//==============================================
// https://www.codewars.com/kata/582887f7d04efdaae3000090You will be given a sequence of objects representing data about developers who have signed up to attend the next coding meetup that you are organising.
...
function findSenior(list) {
    let maxAge = 0
    for(let dev of list){
        maxAge = Math.max(maxAge, dev.age)
    }

    let res = []
    for(let dev of list){
        if(dev.age === maxAge) res.push(dev)
    }
