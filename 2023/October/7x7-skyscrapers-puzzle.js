...
// Some loops were merge, backtrack() now takes an argument to start later in the grid, avoiding useless steps. Overall very similar to the 6x6 approach.
...
//=============================== THIRD APPROACH ===============================
// We are keeping the 1D array. We are keeping the idea of starting our backtrack from the mask with the least possible heights except we are searching for this mask on each backtrack attempt.
// In addition we will have a visited[] Array to target where an investigation needs to be done.

function solvePuzzleV3(clues){
    const N = clues.length / 4
    const MASK = (1 << N) - 1
    let possible = Array(N*N).fill(MASK)
    ...
    //Our backtrack will try to set first the mask with the least amount of possibilities.
    backtrack()
    ...
    function backtrack(){
        ...
        //Searching for the mask with the least possible heights : our backtrack will try to set this element first
        for(let i=0 ; i<N*N ; i++){
        ...
        //if no shifts worked, also handles the case where possible[curr] === 0
        return false
    }
...
// console.table(solvePuzzleV3([0,2,3,0,2,0,0, 5,0,4,5,0,4,0, 0,4,2,0,0,0,6, 5,2,2,2,2,4,1]))
// [ [7,6,2,1,5,4,3],
// [1,3,5,4,2,7,6],
// [6,5,4,7,3,2,1],
// [5,1,7,6,4,3,2],
...
// console.table(solvePuzzleV3([0,2,3,0,2,0,0, 5,0,4,5,0,4,0, 0,4,2,0,0,0,6, 0,0,0,0,0,0,0]))
// [ [7,6,2,1,5,4,3],
// [1,3,5,4,2,7,6],
// [6,5,4,7,3,2,1],
// [5,1,7,6,4,3,2],
...
//hard puzzle
// console.table(solvePuzzleV3([0,0,0,5,0,0,3, 0,6,3,4,0,0,0, 3,0,0,0,2,4,0, 2,6,2,2,2,0,0]))
//[ [3,5,6,1,7,2,4],
// [7,6,5,2,4,3,1],
// [2,7,1,3,6,4,5],
// [4,3,7,6,1,5,2],
...
//very hard puzzle
// console.table(solvePuzzleV3([0,0,5,0,0,0,6, 4,0,0,2,0,2,0, 0,5,2,0,0,0,5, 0,3,0,5,0,0,3]))
// console.log(JSON.stringify(solvePuzzle([0,0,5,0,0,0,6, 4,0,0,2,0,2,0, 0,5,2,0,0,0,5, 0,3,0,5,0,0,3])))
//[ [3,4,1,7,6,5,2],
// [7,1,2,5,4,6,3],
// [6,3,5,2,1,7,4],
...
//very hard puzzle
// console.table(solvePuzzleV3([0,0,5,3,0,2,0, 0,0,0,4,5,0,0, 0,0,0,3,2,5,4, 2,2,0,0,0,0,5]))
// console.log(JSON.stringify(solvePuzzle([0,0,5,3,0,2,0, 0,0,0,4,5,0,0, 0,0,0,3,2,5,4, 2,2,0,0,0,0,5])))
//[ [2,3,1,4,6,5,7],
// [1,7,4,6,5,2,3],
// [3,6,5,7,2,1,4],
