...
console.log("res 795:",solve([ 3, 2, 2, 3, 2, 1, 1, 2, 3, 3, 2, 2, 5, 1, 2, 2, 4, 3, 3, 2, 1, 2, 2, 4])) //[[ 2, 1, 4, 3, 5, 6], [ 1, 6, 3, 2, 4, 5], [ 4, 3, 6, 5, 1, 2], [ 6, 5, 2, 1, 3, 4], [ 5, 4, 1, 6, 2, 3], [ 3, 2, 5, 4, 6, 1]]
...
function solveHUH(clues){
                    ...
                    for(let shift=0 ; shift<N ; shift++){
                        ...
                        if((1 << shift) & possible[row][col]){
                            ...
                            if(isValid() && backtrack()){
                                ...
                                setValue(row, col, 1 << shift, true) // Remove the value before backtracking
    ...
    //This function sets a height, removing this possibility in the row and col as necessary
    function setValue(row, col, mask, remove = false){
        for(let i=0 ; i<N ; i++){
            if (!remove) {
                possible[row][i] &= ~mask
                possible[i][col] &= ~mask
        ...
        possible[row][col] = remove ? MASK : mask
    }
...
console.log("res :",solveHUH([ 3, 2, 2, 3, 2, 1, 1, 2, 3, 3, 2, 2, 5, 1, 2, 2, 4, 3, 3, 2, 1, 2, 2, 4])) //[[ 2, 1, 4, 3, 5, 6], [ 1, 6, 3, 2, 4, 5], [ 4, 3, 6, 5, 1, 2], [ 6, 5, 2, 1, 3, 4], [ 5, 4, 1, 6, 2, 3], [ 3, 2, 5, 4, 6, 1]]
                ...
                if(grid[row][col] === 0){
                    for(let num=1 ; num<=9 ; num++){
                        if(isValid(row, col, num)){
                            grid[row][col] = num
                            let gridCpy = grid.map(subarr => subarr.slice())
                            if(solve()){
                                return true
                            }
                            // else grid[row][col] = 0
...
// console.log(JSON.stringify(sudokuSolver(sudoku))) // [[5,3,4,6,7,8,9,1,2],[6,7,2,1,9,5,3,4,8],[1,9,8,3,4,2,5,6,7],[8,5,9,7,6,1,4,2,3],[4,2,6,8,5,3,7,9,1],[7,1,3,9,2,4,8,5,6],[9,6,1,5,3,7,2,8,4],[2,8,7,4,1,9,6,3,5],[3,4,5,2,8,6,1,7,9]]
// console.log(JSON.stringify(sudokuSolver(sudoku2))) // [[4,9,7,8,5,6,3,2,1],[8,2,3,1,9,7,6,5,4],[5,1,6,4,3,2,7,9,8],[6,3,1,5,7,8,9,4,2],[7,4,9,2,1,3,5,8,6],[2,8,5,9,6,4,1,3,7],[9,5,4,7,8,1,2,6,3],[3,7,8,6,2,5,4,1,9],[1,6,2,3,4,9,8,7,5]]