...
function shiftedDiff(first,second){
    ...
    for(let i=0 ; i<first.length ; i++){
        ...
        shifted = shiftByOne(shifted)
    }

    return -1

    // A shift is putting the last character of a string in front
    function shiftByOne(string){
        if(string.length === 1) return string
        let res = string[string.length-1]
...
function shiftedDiffBis(first, second){
    return (first.length === second.length) ? (second + second).indexOf(first) : -1
}

// console.log(shiftedDiffBis("coffee", "eecoff")) // 2
// console.log(shiftedDiffBis("eecoff","coffee")) // 4
// console.log(shiftedDiffBis("isn't","'tisn")) // 2
...
function sumOfN(n) {
    ...
    return Array.from({length : Math.abs(n) + 1}, (e, i) => i === 0 ? 0 : sign * sumOneToN(i))
