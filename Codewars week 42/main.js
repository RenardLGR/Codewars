...
function validParenthesesBis(parens){
    let n = 0;
...
// It should calculate the area of a regular polygon of numberOfSides sides inside a circle of radius circleRadius which passes through all the vertices of the polygon (such circle is called circumscribed circle or circumcircle). The answer should be a number rounded to 3 decimal places.
...
function areaOfPolygonInsideCircle(circleRadius, numberOfSides) {
    // https://www.editions-petiteelisabeth.fr/calculs_perimetre_aire_polygone_regulier_2.php
    // https://www.accromaths.fr/comment-calculer-laire-dun-polygone/
...
// console.log(everyCombination(3));
// console.log(everyCombination(5));
...
function runSchedule(speed, duration){

    let maxSequence = []
    let maxD = 0


    createSequence(duration, speed, [])
    // console.log(maxSequence, maxD);
    return maxD

    //Helper functions

    //Create a sequence of R and S folowing the rules
    function createSequence(duration, speed, inProgress){
        if(duration===0){
            let d = distanceCovered(inProgress, speed)
            if(d>maxD){
                maxD = d
                maxSequence = inProgress.slice()
            }
        }else{
            if(inProgress.slice(-1)==='S'){
                createSequence(duration-1, speed, [...inProgress, 'R'])
            }else{
                createSequence(duration-1, speed, [...inProgress, 'R'])
                createSequence(duration-1, speed, [...inProgress, 'S'])
            }
        }
    }

    //Given a sequence of run/sprint and a speed returns the total distance covered
    function distanceCovered(sequence, speed){
        let d=0
        let s=speed
        sequence.forEach(e => {
            if(e==='R'){
    ...
    //console.log(distanceCovered(['S', 'R', 'R', 'S'], 2)) // =>8

}

// console.log(runSchedule(2,4)) // => 10, sequence [ 'R', 'R', 'R', 'S' ]
//Probably works but takes too much time for long distance input (as expected)
